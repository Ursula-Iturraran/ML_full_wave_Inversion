function wz=kennetA(lyr,wvlt,dt,mopt,fs,disp)
...
n=length(wvlt); om=(2*pi/(n*dt))*[0:1:(n/2 -1)]; freq=om/(2*pi);
p0=ifft(wvlt); w0=(1/(ro(1)*v(1)))*p0;
rdhat=zeros(size(om)); tdhat=ones(size(om));
      j=2:nlr;
...
xlabel('freq. Hz'), ylabel('|R(f)|'); drawnow
  ...
  fltr=hanning(floor(n/2)); fltr=[ones(floor(n/4),1);fltr(floor(n/4)+1:floor(n/2))]'; 
...
wz=real(fft(wz));
if nargin==5
figure(1),plot([0:dt:dt*(length(wz)-1)],wz),title('reflected seismogram');
end;
